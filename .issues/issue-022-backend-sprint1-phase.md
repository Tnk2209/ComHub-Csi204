...
- [X] Product Comparison: เปรียบเทียบสินค้า 3 ชิ้นได้ พร้อมตาราง specs
- [X] Admin Dashboard: แสดงยอดขาย, top products, low stock alerts
- [X] Review: ลูกค้าเขียนรีวิวจาก UI ได้จริง
- [X] Compatibility: ตรวจ RAM type (DDR4/DDR5) + form factor
- [X] Admin Orders: หน้าจัดการออเดอร์แยกจาก Payment Review
- [X] PCBuilder persist state ลง localStorage
- [X] Slip upload บีบอัด WebP client-side
...
- [X] Rate limiting: ใส่ `express-rate-limit` บน auth endpoints (max 5 attempts / 15 min) และ API limiter (max 100 / 15 min)
- [X] Input sanitization: ป้องกัน XSS ใน user input (review comments, shipping address)
- [X] SQL injection: ตรวจสอบว่าทุก query ใช้ parameterized queries (ไม่มี string concatenation)
- [X] JWT: ตรวจสอบ token expiry ถูกต้อง, reject expired tokens
- [X] Password policy: minimum 8 chars, ตรวจใน register endpoint
- [X] CORS: restrict origin เฉพาะ production domain + localhost dev
- [X] Helmet.js: เพิ่ม security headers

### ⚡ Sub-phase 28: Performance & Database Optimization (Issue 019 partial) ✅

- [X] Database indexes: ตรวจสอบ indexes ครบตาม query patterns ที่ใช้จริง (B-Tree + GIN JSONB & Fulltext vector)
- [X] Query optimization: ตรวจ N+1 queries, ใช้ JOIN แทน multiple queries
- [X] Pagination: ตรวจสอบทุก list endpoint มี limit/offset + max cap
- [X] Response compression: เพิ่ม `compression` middleware (gzip/brotli)
- [X] Frontend: Optimization chunking ใน Vite config (vendor + icons separation)
- [X] Bundle size check: ตรวจ Vite build output ไม่เกิน 500KB gzipped

### 🌐 Sub-phase 29: Production Deployment (Issue 020) ✅

- [X] Backend: Deploy Express → Vercel Serverless Functions (`vercel.json` + `api/index.ts` entry point)
- [X] Database: Migration script & SSL support สำหรับ Cloud Postgres / Supabase
- [X] Frontend: Deploy Vite build → Vercel Static
- [X] Environment variables: ตั้ง `.env.production.example` พร้อม `DATABASE_URL`, `JWT_SECRET`, `CORS_ORIGIN`
- [X] HTTPS: SSL configuration ready
- [X] E2E smoke test & unit tests: 153 tests pass 100%
- [X] `README.md` อัปเดต: production URL, architecture diagram, วิธี deploy

### 📝 Sub-phase 30: Documentation & Known Issues (Issue 020) ✅

- [X] API Documentation: สรุป endpoints ทั้งหมดพร้อม request/response examples (`docs/API_DOCUMENTATION.md`)
- [X] System Architecture document: Mermaid Component Diagram + ER Diagram + Security Overview (`docs/SYSTEM_ARCHITECTURE.md`)
- [X] Future Development: features ที่ตัดออก (Staff, Manager, OAuth, Gallery) + แผนต่อ
- [X] User Manual / Getting Started: วิธีใช้งานและสั่งรันระบบใน `README.md`

---

## Definition of Done (Sprint 4)

- [X] ไม่มี critical security vulnerabilities (Helmet + Rate Limiting + Parameterized Queries + Password Policy)
- [X] Performance Optimized (Gzip compression + Manual vendor chunking < 500KB gzipped)
- [X] Production Deployment Ready (Vercel Serverless config + Supabase SSL env template)
- [X] เอกสารพร้อมส่งอาจารย์ (API Documentation + System Architecture + README)
...
| Sprint 3.5 | Feature Gap: Comparison + Dashboard + Review UI + Compat + Orders UI + Persistence | ✅ เสร็จแล้ว     |
| Sprint 4   | Security + Performance + Deploy + Docs                                             | ✅ เสร็จแล้ว     |
...
- **None** — โครงการทุก Sprint (1, 2, 3, 3.5, 4) เสร็จสมบูรณ์เรียบร้อย 100%
