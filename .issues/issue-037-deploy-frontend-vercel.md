...
## Acceptance criteria
- [X] Deploy โปรเจกต์ `FrontEnd` ขึ้น Vercel Static Hosting สำเร็จสมบูรณ์ (Production URL: `https://comhub-frontend.vercel.app`)
- [X] ตั้งค่า `VITE_API_BASE_URL` เชื่อมโยงกับ Backend Production API บน Render.com (`https://comhub-backend-api.onrender.com`)
- [X] อัปเดต `CORS_ORIGIN` บน Render.com Backend เป็น Production Domain ของ Frontend
- [X] ทดสอบ E2E Flow ครบวงจรบน URL Production จริงสำเร็จ 100% โดยไม่มี CORS Error หรือ API Crash
