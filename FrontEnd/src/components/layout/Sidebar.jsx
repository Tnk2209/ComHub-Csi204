...
function Sidebar({ role, activeMenu, onSelectMenu, onLogout }) {
  ...
  const getMenuItems = () => {
    switch (role) {
      case 'Admin':
        return [
          { id: 'dashboard', name: t('sidebar.dashboard', 'Dashboard'), icon: LayoutDashboard },
          { id: 'products', name: t('sidebar.products', 'Products CRUD'), icon: Database },
          { id: 'accounts', name: t('sidebar.accounts', 'Account Controls'), icon: Users },
          { id: 'payments', name: t('sidebar.payments', 'Payment Review'), icon: CreditCard },
          { id: 'orders', name: t('sidebar.orders', 'Order Management'), icon: ShoppingBag },
        ];
      default:
        return [];
    }
  ...
  return (
    <aside className="w-64 bg-app-surface border-r border-app-border flex flex-col min-h-[calc(100vh-4rem)] transition-colors">
      ...
      <div className="p-4 border-b border-app-border bg-app-bg/50">
        <span className="text-xs text-app-text-muted uppercase tracking-wider block font-semibold">{t('sidebar.logged_in_as', 'Logged in as')}</span>
      ...
      <div className="p-4 border-t border-app-border space-y-1">
        ...
        >
          ...
          {t('sidebar.back_to_store', 'Back to Store')}
        </button>
        {onLogout && (
          <button
            onClick={onLogout}
